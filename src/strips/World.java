...
public class World {
	...
	public boolean goalSatisfied(ArrayList<Condition> curr_state) {
		...
		for(Condition c :curr_state) {
			if (c.name.equals(this.goal.name) && c.params.size()==this.goal.params.size()) {
				if(c.params.isEmpty()) return true;
				for(int i =0;i<c.params.size();i++) {if(c.params.get(i).equals(this.goal.params.get(i))==false ) return false;}
				return true;
			}
		}
		 return false;
	...
	public void initLine(String line) throws Exception {
		...
		if (m == null)
			...
			for(Condition c :state) { if(c.params!=null)
				for (String p :c.params) addLiteral(p);}
	...
	public Condition generateCondition(String s) {
		...
		if (s.endsWith(")")) {
			...
			for (String l : lits) {
				...
				if(a!=null && a.params.contains(l.strip())==false) {   addLiteral(l);}
				tmp.add(l);
			}
	...
	public boolean possibleActions(ArrayList<Condition> pre,ArrayList<Condition> curr_state) {
		for (Condition c :pre) {
			if(!curr_state.contains(c)) { return false;}
		...
		try (BufferedReader br = new BufferedReader(new FileReader(file))) {
			...
			while ((line = br.readLine()) != null) {
				...
				if (parse.equals("init"))
					...
					if (m== null)
						throw new Exception("Action declaration line not found , please write action_declaration(params):");
					...
					if (action.endsWith(")")) {
						...
						a= new Action(name,tmp,new ArrayList<Condition>(),new ArrayList<Condition>(),new ArrayList<Condition>());
					} else
						a= new Action(name,null,new ArrayList<Condition>(),new ArrayList<Condition>(),new ArrayList<Condition>());
					parse="PRE";
				}
